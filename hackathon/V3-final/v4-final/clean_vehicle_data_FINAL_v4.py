...
Fixes applied over original:
  - BUG FIX: Snapshot missing masks BEFORE imputation so _flag_imputed is accurate
  - BUG FIX: Added _flag_imputed column (was completely missing in original)
  - _flag_imputed: True if ANY field was filled in by the pipeline
  - _flag_non_imputable: True if Make/Model/Trim/Body all NULL with no VIN (unchanged)
"""
...
print('='*70)
print('VEHICLE DATA FORENSIC CLEANING - FAST VERSION (FIXED FLAGS)')
print('='*70)

INPUT_FILE  = sys.argv[1] if len(sys.argv) > 1 else '/home/mennatullah/Documents/repos/li/AI/instant/hackathon/data/raw/VehicleSales.parquet'
...
# ── Snapshot which rows are missing BEFORE any imputation ─────────────────────
# BUG FIX: Original script had no _flag_imputed at all.
# We must snapshot here — after imputation the NaNs are gone and we can't recover
# which rows were originally missing.
print('\n  Snapshotting pre-imputation missingness...')
imputable_cols = ['Make', 'Model', 'Trim', 'Body', 'Color', 'Interior',
                  'Transmission', 'Odometer']
imputable_cols = [c for c in imputable_cols if c in df.columns]
pre_imputation_missing = df[imputable_cols].isna().copy()  # bool DataFrame
...
# ── Phase 5: Mode Imputation for Color/Interior ───────────────────────────────
...
# ── Phase 6: Build Flags ──────────────────────────────────────────────────────
# BUG FIX: _flag_imputed now compares pre-imputation snapshot against current state.
# A row is flagged imputed if it was missing before AND is now filled in.
print('\n[7/7] Phase 7: Flagging rows...')
with Timer('FLAG imputed + non-imputable'):

    # _flag_imputed: was missing before, now has a value
    ...
    # _flag_non_imputable: Make/Model/Trim/Body all still NULL with no valid VIN
    ...
    # Sanity check: a row shouldn't be both imputed and non-imputable
    overlap = (imputed_any & non_imp).sum()
    if overlap:
        print(f'  ⚠ WARNING: {overlap} rows are flagged as BOTH imputed and non-imputable — investigate!')
    else:
        print('  ✓ Sanity check passed: no overlap between imputed and non-imputable flags')
...
print(f'Clean (no changes):   {(~df["_flag_imputed"] & ~df["_flag_non_imputable"]).sum():,}')
