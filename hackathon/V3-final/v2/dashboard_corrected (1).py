...
# ─── Load & prepare data ───────────────────────────────────────────────────
RAW_PATH = "VehicleSales.parquet"  # update this path to your parquet file

df = pd.read_parquet(RAW_PATH)

# ─── FIX: Column-shifted rows ──────────────────────────────────────────────
# 26 rows have values shifted right by 1 column from Body onward.
# Transmission contains "sedan"/"Sedan" (actually the Body type),
# VIN contains "automatic"/None (actually the Transmission),
# State contains the real VIN. Body contains garbage ("Navitgation").
shift_mask = df["Transmission"].isin(["sedan", "Sedan"])
if shift_mask.sum():
    df.loc[shift_mask, "Body"] = "Sedan"                    # Transmission held body type
    df.loc[shift_mask, "Transmission"] = df.loc[shift_mask, "VIN"]   # VIN held transmission
    df.loc[shift_mask, "VIN"] = df.loc[shift_mask, "State"]          # State held VIN
    df.loc[shift_mask, "State"] = np.nan                    # Original state unrecoverable
